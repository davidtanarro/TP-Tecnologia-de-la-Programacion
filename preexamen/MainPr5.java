...
public class MainPr5 {
	...
	public static void setGameFactory(GameFactory gameFactory) {
		MainPr5.gameFactory = gameFactory;
	}
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case TIC_TAC_TOE:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				gameFactory = new TicTacToeFactoryExt(dimRows);
			} else {
		...
		case ADVANCED_TIC_TAC_TOE:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				gameFactory = new AdvancedTTTFactoryExt(dimRows);
			} else {
		...
		case ATAXX:
			...
			else if(numAtaxxObstacles != null){
				gameFactory = new AtaxxFactoryExt(numAtaxxObstacles);
			}
			else if(dimRows != null && dimCols != null && dimRows == dimCols && numAtaxxObstacles != null){
	...
	private static void parseHelpOption(CommandLine line, Options cmdLineOptions) {
		if (line.hasOption("h")) {
			...
			formatter.printHelp(MainPr5.class.getCanonicalName(), cmdLineOptions, true);
			System.exit(0);
		}
	}
