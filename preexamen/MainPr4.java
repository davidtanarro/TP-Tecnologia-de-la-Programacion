...
import es.ucm.fdi.tp.examen.aataxx.AAtaxxFactory;
...
public class MainPr4 {
	 ...
	 */
	enum GameInfo {
		CONNECTN("cn", "ConnectN"), TicTacToe("ttt", "Tic-Tac-Toe"), AdvancedTicTacToe("attt", "Advanced Tic-Tac-Toe"), Ataxx("ataxx", "Ataxx"), AdvancedATAXX("aataxx","Advanced-Ataxx");
	 ...
	 * Juego por defecto.
	 */
	final private static GameInfo DEFAULT_GAME = GameInfo.CONNECTN; // GameInfo.Ataxx
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case TicTacToe:
			...
			break;
		case Ataxx:
			if (dimRows != null && dimCols != null && numObs != null)
				gameFactory = new AtaxxFactory(dimRows, dimCols, numObs);
			else if (dimRows != null && dimCols != null)
				gameFactory = new AtaxxFactory(dimRows, dimCols);
			else if (numObs != null)
				gameFactory = new AtaxxFactory(numObs);
			else
		...
		case AdvancedATAXX:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				if (numObs == null) {
					gameFactory = new AAtaxxFactory(dimRows, dimCols);
				} else {
					gameFactory = new AAtaxxFactory(dimRows, dimCols, numObs);
				}
			} else {
				if (numObs == null) {
					gameFactory = new AAtaxxFactory();
				} else {
					gameFactory = new AAtaxxFactory(numObs);
				}
			}
			break;
		default:
	...
	private static Option constructObstacleOption() {
		return new Option ("o", "obstacles", true,
				"The number of squares occupied by obstacles (if allowed by the selected game).");
	}
	
	// Parse the option -o or --obstacles
	private static void parseObstaclesOption(CommandLine line) throws ParseException {
		String obsVal = line.getOptionValue("o");
		
		if (obsVal != null) {
			try {
				numObs = Integer.parseInt(obsVal);
			}
			catch (NumberFormatException e){
	...
	private static void parseHelpOption(CommandLine line, Options cmdLineOptions) {
		if (line.hasOption("h")) {
			...
			formatter.printHelp(MainPr4.class.getCanonicalName(), cmdLineOptions, true);
			System.exit(0);
		}
	}
