...
public class MainPr6 {
	...
	enum GameInfo {
		ATAXX("ataxx", "Ataxx"), CONNECTN("cn", "ConnectN"), TicTacToe("ttt", "Tic-Tac-Toe"), AdvancedTicTacToe("attt",
				"Advanced Tic-Tac-Toe"), AdvancedATAXX("advancedAtaxx","Advanced Ataxx");
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case AdvancedATAXX:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				if (numAtaxxObstacles == null) {
					//gameFactory = new AAtaxxFactoryExt(dimRows, dimCols);
				} else {
					//gameFactory = new AAtaxxFactoryExt(dimRows, dimCols, numAtaxxObstacles);
				}
			} else {
				if (numAtaxxObstacles == null) {
					//gameFactory = new AAtaxxFactoryExt();
				} else {
					//gameFactory = new AAtaxxFactoryExt(numAtaxxObstacles);
				}
			}
			break;
		default:
	...
	private static void parseHelpOption(CommandLine line, Options cmdLineOptions) {
		if (line.hasOption("h")) {
			...
			formatter.printHelp(MainPr6.class.getCanonicalName(), cmdLineOptions, true);
			System.exit(0);
		}
	}
